...
def load_models_and_data():
    ...
    try:
        ...
        with st.spinner("Loading transformer model from Hugging Face..."):
            tokenizer, model, sentiment_pipeline = load_transformer_model_from_hf(HUGGING_FACE_MODEL_NAME)
            
            if tokenizer and model and sentiment_pipeline:
                models['transformer_tokenizer'] = tokenizer
                models['transformer_model'] = model
                models['sentiment_pipeline'] = sentiment_pipeline
            else:
                st.warning("Transformer model could not be loaded from Hugging Face. Transformer predictions will not be available.")
        ...
        try:
            ...
            data['predicted_labels_transformer_str'] = [label_map_transformer_to_str.get(label, 'unknown') for label in transformer_predicted_labels]
             ...
             st.warning(f"Some confusion matrix data could not be loaded: {e}")
...
if models:
    ...
    if data and 'y_test_std' in data and 'lr_pred_std' in data and 'nb_pred_std' in data and 'svm_pred_std' in data and \
        ...
        col_transformer_plot, col_spacer1, col_spacer2 = st.columns(3)
        ...
        if user_input:
            ...
            try:
                if 'sentiment_pipeline' in models and models['sentiment_pipeline']:
                    ...
                    # Handle different label formats from Hugging Face models
                    label = transformer_result['label'].upper()
                    score = transformer_result['score']
                    
                    # Map common label formats to standard format
                    if 'POSITIVE' in label or 'POS' in label or label == 'LABEL_1':
                        ...
                        try:
                            label_str = label.replace("LABEL_", "")
                    ...
                    results['Transformer (Hugging Face)'] = {'prediction': 'Not Loaded', 'confidence': 0.0}

            except Exception as e:
                st.error(f"Error with Transformer prediction: {e}")
                results['Transformer (Hugging Face)'] = {'prediction': 'Error', 'confidence': 0.0}
